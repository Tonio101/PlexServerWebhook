...
def config_logger():
    ...
    rotate_file_handler = \
        RotatingFileHandler(LOG_FILE, maxBytes=LOG_FILE_MAXSIZE,
...
def init():
    ...
    usage = ("{FILE} "
             "--phone 1234567890 "
             "--host 0.0.0.0 "
             "--port 6669").format(FILE=__file__)
    description = ("Start webhook server for Plex and notifiy "
                   "user via text message")
    parser = argparse.ArgumentParser(usage=usage, description=description)
    parser.add_argument("-p", "--phone",
                        help="Phone number (T-Mobile only for now)",
                        required=True)
    parser.add_argument("--host",
                        help="Webhook server IP",
                        required=False)
...
def respond():
    ...
    message = message.format(current_user.event, current_user.user,
                             current_user.title,
                             current_user.player['publicAddress'],
                             current_user.player['title'])
    cmd_str = \
        ("echo \"{MSG}\" | mail -s 'Plex Server Webhook' {PHONE}@tmomail.net")\
        .format(MSG=message, PHONE=PHONE_NUMBER)
